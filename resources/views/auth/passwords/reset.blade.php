...
<div class="container mx-auto flex flex-col px-16 w-128 border border-green-lighter shadow-login mt-24">
    ...
    <form method="POST" action="{{ route('password.update') }}">
        ...
        <div>
            <input id="email" type="email" name="email" value="{{ $email ?? old('email') }}" class="border-b w-full text-green-light font-bold focus:outline-none focus:border-green"
                required autofocus>
        </div>
        <div class="h-10 pt-1">
            @if ($errors->has('email'))
        ...
        <div>
            <button type="submit" class="py-3 w-full mb-16 bg-green text-lg text-white font-semibold uppercase focus:outline-none">Reset Password</button>
        </div>
    </form>
</div>
@endsection